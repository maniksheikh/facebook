<template>
  <div>
    <div v-for="(post, index) in posts" :key="index" class="permanent-section">
      <div class="items">
        <div class="post-content">
          <div>
            <img class="post-profile-img" :src="post.profileImg" />
            <p>{{ post.name }}</p>
          </div>
          <div class="multi-icon">
            <img src="../assets/image/icons8-more-24.png" />
            <img src="../assets/image/icons-multiply-img.png" />
          </div>
        </div>
        <div class="post-title">
          <h3>{{ post.title }}</h3>
        </div>
        <hr />
        <img class="post-img" :src="post.postImg" />
        <div class="react-btn">
          <button class="btn btn-react">
            <svg
              id="Capa_1"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              x="0px"
              width="20px"
              height="20px"
              y="0px"
              viewBox="0 0 486.926 486.926"
              style="enable-background: new 0 0 486.926 486.926"
              xml:space="preserve"
            >
              <g>
                <path
                  d="M462.8,181.564c-12.3-10.5-27.7-16.2-43.3-16.2h-15.8h-56.9h-32.4v-75.9c0-31.9-9.3-54.9-27.7-68.4
          c-29.1-21.4-69.2-9.2-70.9-8.6c-5,1.6-8.4,6.2-8.4,11.4v84.9c0,27.7-13.2,51.2-39.3,69.9c-19.5,14-39.4,20.1-41.5,20.8l-2.9,0.7
          c-4.3-7.3-12.2-12.2-21.3-12.2H24.7c-13.6,0-24.7,11.1-24.7,24.7v228.4c0,13.6,11.1,24.7,24.7,24.7h77.9c7.6,0,14.5-3.5,19-8.9
          c12.5,13.3,30.2,21.6,49.4,21.6h65.9h6.8h135.1c45.9,0,75.2-24,80.4-66l26.9-166.9C489.8,221.564,480.9,196.964,462.8,181.564z
           M103.2,441.064c0,0.4-0.3,0.7-0.7,0.7H24.7c-0.4,0-0.7-0.3-0.7-0.7v-228.4c0-0.4,0.3-0.7,0.7-0.7h77.9c0.4,0,0.7,0.3,0.7,0.7
          v228.4H103.2z M462.2,241.764l-26.8,167.2c0,0.1,0,0.3-0.1,0.5c-3.7,29.9-22.7,45.1-56.6,45.1H243.6h-6.8h-65.9
          c-21.3,0-39.8-15.9-43.1-36.9c-0.1-0.7-0.3-1.4-0.5-2.1v-191.6l5.2-1.2c0.2,0,0.3-0.1,0.5-0.1c1-0.3,24.7-7,48.6-24
          c32.7-23.2,49.9-54.3,49.9-89.9v-75.3c10.4-1.7,28.2-2.6,41.1,7c11.8,8.7,17.8,25.2,17.8,49v87.8c0,6.6,5.4,12,12,12h44.4h56.9
          h15.8c9.9,0,19.8,3.7,27.7,10.5C459,209.864,464.8,225.964,462.2,241.764z"
                />
              </g>
            </svg>
            <span>Like</span>
          </button>
          <button class="btn btn-comment">
            <svg
              width="20px"
              height="20px"
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                d="M3.25 4a.25.25 0 00-.25.25v12.5c0 .138.112.25.25.25h2.5a.75.75 0 01.75.75v3.19l3.72-3.72a.75.75 0 01.53-.22h10a.25.25 0 00.25-.25V4.25a.25.25 0 00-.25-.25H3.25zm-1.75.25c0-.966.784-1.75 1.75-1.75h17.5c.966 0 1.75.784 1.75 1.75v12.5a1.75 1.75 0 01-1.75 1.75h-9.69l-3.573 3.573A1.457 1.457 0 015 21.043V18.5H3.25a1.75 1.75 0 01-1.75-1.75V4.25z"
              />
            </svg>
            <span>Comment</span>
          </button>
          <button class="btn btn-share">
            <svg
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              width="20px"
              height="20px"
              x="0px"
              y="0px"
              viewBox="0 0 1000 1000"
              enable-background="new 0 0 1000 1000"
              xml:space="preserve"
            >
              <metadata>
                Svg Vector Icons : http://www.onlinewebfonts.com/icon
              </metadata>
              <g>
                <path d="M104.1,705.2" />
                <path
                  d="M978.7,420.9L581.6,95c-9.2-7.5-21.8-8.9-32.5-3.9c-10.7,5.1-17.5,15.8-17.5,27.7l-0.9,171.9c-178,5.7-315.8,62.9-405.1,167.4C-19.9,628.2,12.6,875.5,14.1,885.8c2.3,15,13.3,26,28.4,26c0.3,0,0.7,0,1.1,0c15.5-0.5,25.3-12.5,26.6-28c1-11.3,31.5-286.2,460.6-285.9l0.9,175.8c0,11.8,6.8,22.6,17.6,27.7c10.6,5.1,23.4,3.5,32.5-4l397.1-329c7.1-5.8,11.2-14.5,11.2-23.7C990,435.4,985.8,426.7,978.7,420.9z M592.9,708.8v-142c0-8.1-3.2-15.9-9-21.7c-5.8-5.7-13.5-8.9-21.6-8.9c0,0-0.1,0-0.1,0c-306.4,0.2-423.5,103.7-490.4,195c9.8-73.2,38.3-161.4,100.5-233.5c82.6-95.8,215.7-146.3,390-146.3c16.9,0,30.6-13.7,30.6-30.6V183.2L911,444.8L592.9,708.8z"
                />
              </g>
            </svg>
            <span>Share</span>
          </button>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      posts: [
        {
          name: 'Manik Sheikh',
          profileImg: require('../assets/image/profile-img.jpg'),
          title: 'Have a good Day in Morning Dhaka.',
          postImg: require('../assets/image/new-post.JPG'),
        },
        {
          name: 'Nasim Uddin',
          profileImg: require('../assets/image/freelancing.jpg'),
          title: 'If you Became a good Developer, You Should Hard Work Everyday.',
          postImg: require('../assets/image/freelancing.jpg'),
        },
        {
          name: 'Friends Group',
          profileImg: require('../assets/image/friends.jpg'),
          title: 'Have a Good Day With My Friends.',
          postImg: require('../assets/image/friends.jpg'),
        },
        {
          name: 'Traveling Journey',
          profileImg: require('../assets/image/iceland.jpg'),
          title: 'Beautiful Places Iceland - Espy Travel & Tourism in The World.',
          postImg: require('../assets/image/iceland.jpg'),
        },
        {
          name: 'Travelling Journey',
          profileImg: require('../assets/image/profile-img.jpg'),
          title: "It's a Very Amazing Place in Morning, When Sun is Rising.",
          postImg: require('../assets/image/beautiful place.jpg'),
        },
      ],
    }
  },
  computed: {
    items() {
      return this.$store.getters.items
    },
  },
  created() {
    this.$store.dispatch('initItems')
  },
}
</script>

<style lang="scss">
.permanent-section {
  background: #ffffff;
  border: 1px rgb(233, 225, 225) solid;
  border-radius: 10px;
  transition: none !important;
  transform: none !important;
  margin-bottom: 1.5rem; 

  .items {
    font-family: inherit;
    transition: none !important;
    transform: none !important;

    .post-content {
      width: 100%;
      display: flex;
      align-items: center;
      justify-content: space-between;

      div {
        display: flex;
        padding: 10px;
        .post-profile-img {
          border: 1px solid #ccc;
          width: 50px;
          height: 50px;
          border-radius: 50%;
          cursor: pointer;
        }
        p {
          align-items: start;
          margin: auto;
          font-size: 18px;
          font-weight: 600;
          padding-left: 12px;
          cursor: pointer;
        }
      }

      .multi-icon {
        display: flex;
        justify-content: space-between;
        cursor: pointer;
        img {
          height: 26px;
          width: 26px;
          color: #f2f2f2 !important;
          opacity: 0.5;
        }
      }
    }

    .post-title {
      padding-left: 1rem;
      font-weight: 600;
      opacity: 0.7;
      margin-top: -6px;
    }
    .post-img {
      height: 650px;
      width: 100%;
      background-position: initial;
      background-repeat: no-repeat;
      background-size: cover;
      object-fit: cover;
    }
  }
}

/* Updated Hover Styles */
.permanent-section button:hover,
.permanent-section button:active,
.permanent-section button:focus,
.permanent-section .btn:hover,
.permanent-section .btn:active,
.permanent-section .btn:focus {
  background-color: #f0f2f5 !important;
  transform: none !important;
  box-shadow: none !important;
  transition: none !important;
}

@media screen and (max-width: 800px) {
  .permanent-section {
    button:last-child {
      display: none;
    }
  }
}
</style>